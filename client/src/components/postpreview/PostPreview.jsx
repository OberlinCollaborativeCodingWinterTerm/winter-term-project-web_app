        ...
        return (
            <Container className="container-md bg-white p-4">
                <p className="mb-1 text-secondary">
                  <small>
                    <img alt="" src={this.props.userIcon} style={{ width: '1.5em', height: '1.5em' }} className="rounded-circle"  /> 
                    <span className="px-1"> 
                      <Link to={"/profile/" + this.props.userId}>{this.props.userName}</Link>
                    </span>
                    <Badge pill bg="primary">STUDENT</Badge>
                    <span className="px-1">&middot; posted in <Link to={"/classes/" + this.props.classId}>{this.props.classId}</Link></span>
                  </small>
                </p>
